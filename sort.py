    ...
    if len(file.rsplit(".", 1)) > 1 and\
        ...
        try:
            with open(file, 'rb') as fh:
                tags = exifread.process_file(fh, stop_tag="Image Model")
                cmod = tags["Image Model"].printable
        except:
            cmod = "Generic_Model"
        try:
            with open(file, 'rb') as fh:
                tags = exifread.process_file(fh, stop_tag="Image Make")
                make = tags["Image Make"].printable
        except:
            make = "Generic_Make"

        source = make.strip().replace(" ", "_") + "_" + cmod.strip().replace(" ", "_")
